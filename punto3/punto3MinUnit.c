...
MU_TEST(test_MatrizCuadrada2PS) {
    float matriz[3][4] = {
            ...
            {2, 3, 2, 2},
            {1, 2, 0, 1}
    };// El 10 y el 10 son los ptos de silla (mayor en columna, menor en fila)
    RESULTADOS res = puntos_de_silla(3, 4, matriz);
    mu_assert_int_eq(res.cantidad_resultados_indices, 2);
    mu_assert_int_eq(3, res.indices[0].x + res.indices[1].x); // 0 + 1 (o viceversa)
    mu_assert_int_eq(0, res.indices[0].y + res.indices[1].y); // 2 + 0 (o viceversa)
}
